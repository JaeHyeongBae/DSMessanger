...
public:
	...
	*/
	...
	/**
	*	@brief	Retrieve list element whose key matches item's ID (if present).
	*	@pre	ID member of item is initialized.
	*	@post	If there is an element whose key matches with item's key then the element's record is copied to the item.
	*	@return	pointer of the searched item in the list
	*/
	T* Retrieve(T& item);

	/**
	*	@brief	Retrieve list element whose key matches paramteter string ID.
	*	@pre	
	*	@post	If there is an element whose key matches with item's key then the element's record's pointer is returned. Else nullptr is returned.
	*	@return	pointer of the searched item in the list
	*/
	T* Retrieve(std::string id);

	/**
	*	@brief	Retrieve list element whose key matches item's Name (if present).
	*	@pre	Name member of item is initialized.
	*	@post	If there is an element whose key matches with item's key then the element's record is copied to the item.
	*	@return	1 if any element's primary key matches with item's, otherwise 0.
	*/
	T* RetrieveByName(std::string name);
...
// Retrieve list element whose key matches item's key (if present).
template <typename T>
T* SortedLinkedList<T>::Retrieve(T& item)
{
	bool moreToSearch, found;
	NodeType<T>* location;
...
template<typename T>
inline T * SortedLinkedList<T>::Retrieve(std::string id)
{
	bool moreToSearch, found;
	NodeType<T>* location;
...
template<typename T>
inline T* SortedLinkedList<T>::RetrieveByName(std::string name)
{
	bool moreToSearch, found;
	NodeType<T>* location;
